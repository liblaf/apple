...
class Config(cherries.BaseConfig):
    ...
    input: Path = cherries.output(f"10-input-partial{SUFFIX}.vtu")


def extract_points_within_bounds(mesh: pv.UnstructuredGrid) -> pv.UnstructuredGrid:
    mask: Bool[Array, " points"] = melon.bounds_contains(
        jnp.asarray(
            [
                -jnp.inf,
    ...
    mesh = mesh.extract_points(jnp.flatnonzero(mask))  # pyright: ignore[reportArgumentType, reportAssignmentType]
    return mesh
...
def main(cfg: Config) -> None:
    ...
    mesh = extract_points_within_bounds(mesh)
    ic(mesh)
