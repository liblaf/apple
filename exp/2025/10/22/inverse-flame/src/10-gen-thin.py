...
class Config(cherries.BaseConfig):
    ...
    input: Path = cherries.output(f"10-input-thin{SUFFIX}.vtu")


def extract_points_near_face(mesh: pv.UnstructuredGrid) -> pv.UnstructuredGrid:
    face_points: Float[np.ndarray, "face 3"] = mesh.points[mesh.point_data["IsFace"]]
    kdtree = scipy.spatial.KDTree(face_points)
    distance: Float[np.ndarray, " points"]
    distance, _idx = kdtree.query(
        mesh.points,
        k=1,
        distance_upper_bound=1.0,  # centimeters
    )  # pyright: ignore[reportAssignmentType]
    mesh = mesh.extract_points(np.flatnonzero(distance < 1.0))  # pyright: ignore[reportAssignmentType]
    return mesh
...
def main(cfg: Config) -> None:
    ...
    mesh = extract_points_near_face(mesh)
    ic(mesh)
