...
class Config(cherries.BaseConfig):
    ...
    initial: Path = cherries.temp(
        f"20-inverse-adam{SUFFIX}.vtu.d/20-inverse-adam-123k_000999.vtu"
    )
...
def main(cfg: Config) -> None:
    ...
    mesh.cell_data[ACTIVATION] = initial.cell_data[ACTIVATION]
    inverse: PhaceInverse = prepare(mesh, cfg.expression)
    params: PhaceInverse.Params = PhaceInverse.Params(
        activation=jnp.asarray(initial.cell_data[ACTIVATION][inverse.active_cell_id])
    )

    with melon.SeriesWriter(
        cherries.temp(f"20-inverse-adam-stage-2{SUFFIX}.vtu.series")
    ) as writer:
