...
class Config(cherries.BaseConfig):
    ...
    output: Path = cherries.output("20-inverse-no-sparse.vtu")
...
class Inverse:
    ...
    def set_activation(self, activations: Float[Array, "c 6"]) -> None:
        wp.copy(self.energy.params.activations, wpu.to_warp(activations, dtype=vec6))
...
def calc_inverse(
    ...
    with melon.SeriesWriter(
        cherries.temp(f"20-inverse-no-sparse-{idx}.vtu.series")
    ) as writer:
