...
class Config(cherries.BaseConfig):
    ...
    output: Path = cherries.output("20-inverse-sgd.vtu")
...
class InverseActivation(Inverse):
    ...
    def adjoint(self, u: Full, dLdu: Full) -> Full:
        ...
        cherries.log_metrics(
            {"CG_success": int(len(solution.state.state) == 1)}, step=self.step.item()
        )
...
def prepare(mesh: pv.UnstructuredGrid) -> tuple[pv.UnstructuredGrid, InverseActivation]:
    ...
    inverse = InverseActivation(
        ...
        target=target,
        optimizer=Optax(optax.sgd(2.0), max_steps=10000, gtol=jnp.asarray(1e-5)),
    )
    return mesh, inverse
...
def calc_inverse(
    ...
    with melon.SeriesWriter(
        cherries.temp(f"20-inverse-sgd-{idx}.vtu.series")
    ) as writer:
