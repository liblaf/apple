...
class Config(cherries.BaseConfig):
    ...
    output_summary: Path = cherries.output(
        "20-surface-prestrain-expression000-summary.json"
    )
    output_csv: Path = cherries.output("20-surface-prestrain-expression000-cases.csv")
    output_table: Path = cherries.output("20-surface-prestrain-expression000-table.md")
...
@attrs.define
class SurfaceTrianglePrestrain:
    rest_points: Tensor
    triangles: Tensor
    inv_metric: Tensor
    area: Tensor
...
class SurfacePrestrainModel:
    base: Any
    surface: SurfaceTrianglePrestrain
...
def solve_case(case: Case, cfg: Config) -> dict[str, Any]:
    ...
    output_path = cfg.output_summary.parent / f"20-surface-prestrain-{case.name}.vtu"
    surface_path = (
        cfg.output_summary.parent / f"20-surface-prestrain-{case.name}-surface.vtp"
    )
