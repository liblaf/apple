...
class Config(cherries.BaseConfig):
    ...
    output_summary: Path = cherries.output(
        "21-surface-prestrain-expression000-prestrain04-summary.json"
    )
    output_csv: Path = cherries.output(
        "21-surface-prestrain-expression000-prestrain04-cases.csv"
    )
    output_table: Path = cherries.output(
        "21-surface-prestrain-expression000-prestrain04-table.md"
    )
...
def solve_case(case: Case, cfg: Config) -> dict[str, Any]:
    ...
    output_path = (
        ...
        / f"21-surface-prestrain-expression000-prestrain04-{case.name}.vtu"
    )
    surface_path = (
        cfg.output_summary.parent
        / f"21-surface-prestrain-expression000-prestrain04-{case.name}-surface.vtp"
    )
