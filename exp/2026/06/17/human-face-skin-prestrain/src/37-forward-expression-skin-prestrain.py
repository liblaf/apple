...
class ForwardSkinPrestrainConfig(cherries.BaseConfig):
    model_config = ps.SettingsConfigDict(cli_parse_args=True)

    input_mesh: Path = cherries.input(SOURCE_MESH)
    output_summary: Path = cherries.output(
        "37-lips-corners-down-skin-prestrain-forward-summary.json", mkdir=True
    )
    output_result: Path = cherries.output(
        "37-lips-corners-down-skin-prestrain-forward.vtu", mkdir=True
    )
    output_skin: Path = cherries.output(
        "37-lips-corners-down-skin-prestrain.vtp", mkdir=True
    )
    output_skin_inspect: Path = cherries.output(
        "37-lips-corners-down-skin-prestrain-inspect.vtp", mkdir=True
    )
    output_target: Path = cherries.output(
        "37-lips-corners-down-target.vtu", mkdir=True
    )
...
def vector_stats(
    ...
    return stats(prefix, np.linalg.norm(np.asarray(values, dtype=np.float64), axis=1))


def prepare_simulation_mesh(cfg: ForwardSkinPrestrainConfig) -> tuple[pv.UnstructuredGrid, dict[str, Any]]:
...
def make_area_skin(
    ...
    target: np.ndarray,
    cfg: ForwardSkinPrestrainConfig,
    ...
    area_ratio[valid_rest_area] = target_area[valid_rest_area] / rest_area[valid_rest_area]
    ...
    effective_area_ratio = np.maximum(effective_area_ratio, cfg.area_ratio_floor)
    activation_diag = np.sqrt(effective_area_ratio) - 1.0
    ...
    surface.cell_data["StressFreeAreaRatio"] = 1.0 / effective_area_ratio
    surface.cell_data["TargetRestLengthFactor"] = np.sqrt(effective_area_ratio)
...
def make_forward(
    mesh: pv.UnstructuredGrid, skin: pv.PolyData, cfg: ForwardSkinPrestrainConfig
) -> Any:
    ...
    builder.add_potential(Koiter.from_pyvista(skin, name="skin", thickness=SKIN_THICKNESS))
...
def run_skin_only_forward(
    ...
    for name in ("aponeurosis", "fat", "muscle"):
        materials[name]["activation_inv"] = zero_activation
...
def deformation_det(points: np.ndarray, displacement: np.ndarray, tets: np.ndarray) -> np.ndarray:
    X = points[tets]
...
def write_outputs(
    ...
    result.point_data["SkinPrestrainPoint"] = result.points + displacement
    result.point_data["TargetPoint"] = result.points + target
    result.cell_data[ACTIVATION_INV.vtk] = zero_activation
    result.cell_data["ZeroVolumeActivationInv"] = zero_activation
    ...
    surface_ids = np.asarray(skin_inspect.point_data[GLOBAL_POINT_ID.vtk], dtype=np.int64)
    skin_inspect.point_data["TargetDisplacement"] = target[surface_ids]
    skin_inspect.point_data["TargetPoint"] = skin_inspect.points + target[surface_ids]
    skin_inspect.point_data["SkinPrestrainDisplacement"] = displacement[surface_ids]
    skin_inspect.point_data["SkinPrestrainPoint"] = (
        skin_inspect.points + displacement[surface_ids]
    )
...
def run_forward_skin_prestrain(cfg: ForwardSkinPrestrainConfig) -> dict[str, Any]:
    ...
    logger.info("Running skin-prestrain-only forward solve for %s", cfg.target_expression)
    displacement, forward_metrics = run_skin_only_forward(mesh, skin, cfg)
    ...
    summary: dict[str, Any] = {
        **prep_summary,
        ...
        "volume/prestrain": "zero",
        "skin/prestrain_source": "IsFace stretch-only target surface area",
        ...
        **skin_metrics,
        **forward_metrics,
        **vector_stats("forward/displacement_all", displacement),
        **vector_stats("forward/displacement_free", displacement, free),
        **vector_stats("forward/displacement_surface", displacement, surface_mask),
        **vector_stats("forward/displacement_target", displacement, loss_mask),
        **stats("forward/detF", det_f),
        "forward/detF_inverted_cells": int((det_f <= 0.0).sum()),
        "forward/detF_lt_0p5_cells": int((det_f < 0.5).sum()),
        "forward/detF_gt_1p5_cells": int((det_f > 1.5).sum()),
    }
    write_outputs(mesh, skin, target, loss_mask, displacement, cfg)
...
if __name__ == "__main__":
    cherries.main(run_forward_skin_prestrain)
