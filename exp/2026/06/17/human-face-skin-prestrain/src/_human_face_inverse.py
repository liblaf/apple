...
class InverseConfig(cherries.BaseConfig):
    ...
    top_surface_axis: str = "y"
    top_surface_quantile: float = 0.985
    top_surface_displacement_y: float = 0.1
...
def selected_cases(cfg: InverseConfig) -> list[InverseCase]:
    targets = ["smile", "top-y"] if cfg.target == "all" else split_csv(cfg.target)
    invalid = sorted(set(targets) - {"smile", "top-y"})
    if invalid:
        msg = f"unknown target(s) {invalid}; expected smile, top-y, or all"
...
def skin_activation_inv(n_cells: int, prestrain: float) -> np.ndarray:
    activation_inv = np.zeros((n_cells, 3), dtype=np.float64)
    if prestrain != 0.0:
        offset = 1.0 / (1.0 + prestrain) - 1.0
        activation_inv[:, 0] = offset
        activation_inv[:, 1] = offset
...
def skin_surface(mesh: pv.UnstructuredGrid, prestrain: float) -> pv.PolyData:
    ...
    surface.cell_data[ACTIVATION_INV.vtk] = skin_activation_inv(
        surface.n_cells, prestrain
    )
...
def build_forward(
    ...
    if case.skin_enabled:
        skin = skin_surface(mesh, case.skin_prestrain)
...
def top_surface_target(
    mesh: pv.UnstructuredGrid, cfg: InverseConfig
) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    axis_map = {"x": 0, "y": 1, "z": 2}
    if cfg.top_surface_axis not in axis_map:
        msg = f"top_surface_axis must be one of {sorted(axis_map)}, got {cfg.top_surface_axis!r}"
        raise ValueError(msg)
    if not 0.0 <= cfg.top_surface_quantile < 1.0:
        msg = "top_surface_quantile must be in [0, 1)"
        raise ValueError(msg)

    surface, original_ids = surface_original_ids(mesh)
    axis = axis_map[cfg.top_surface_axis]
    surface_coords = np.asarray(mesh.points[original_ids], dtype=np.float64)[:, axis]
    cutoff = float(np.quantile(surface_coords, cfg.top_surface_quantile))
    selected_surface_ids = np.unique(original_ids[surface_coords >= cutoff])
    mask = np.zeros(mesh.n_points, dtype=bool)
    mask[selected_surface_ids] = True
    fixed = np.asarray(mesh.point_data[IS_FIXED], dtype=bool)
    mask &= ~fixed

    target = np.zeros((mesh.n_points, 3), dtype=np.float64)
    target[mask, 1] = cfg.top_surface_displacement_y
    return (
        target,
        mask,
        {
            "target/top_surface_axis": cfg.top_surface_axis,
            "target/top_surface_quantile": float(cfg.top_surface_quantile),
            "target/top_surface_cutoff": cutoff,
            "target/top_surface_surface_points": int(surface.n_points),
            "target/top_surface_selected_points": int(mask.sum()),
            "target/top_surface_displacement_y": float(cfg.top_surface_displacement_y),
        },
    )


def target_displacement_and_mask(
    mesh: pv.UnstructuredGrid, case: InverseCase, cfg: InverseConfig
) -> tuple[np.ndarray, np.ndarray, dict[str, Any]]:
    ...
    elif case.target == "top-y":
        target, mask, extra_metrics = top_surface_target(mesh, cfg)
    else:
...
def solve_case(  # noqa: C901, PLR0915
    ...
    summary: dict[str, Any] = {
        ...
        "n_skin_triangles": 0 if skin is None else int(skin.n_cells),
...
def write_table(path: Path, rows: list[dict[str, Any]]) -> None:
    lines = [
        "| case | setup | target | tets | active | params | loss pts | stop | best step | best loss | error RMS | error/target | disp edge RMS | residual edge RMS | disp lap RMS | residual lap RMS |",
        "| --- | --- | --- | ---: | ---: | ---: | ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |",
    ]
    lines.extend(
        (
...
def run_inverse(cfg: InverseConfig) -> None:
    ...
    summary = {
        ...
        "inverse/max_steps": int(cfg.inverse_max_steps),
        "inverse/loss_min_delta": float(cfg.inverse_loss_min_delta),
        "top_surface_axis": cfg.top_surface_axis,
        "top_surface_quantile": float(cfg.top_surface_quantile),
        "top_surface_displacement_y": float(cfg.top_surface_displacement_y),
    }
    cfg.output_summary.write_text(
        json.dumps(summary, indent=2, sort_keys=True), encoding="utf-8"
    )
