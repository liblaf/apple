...
EXPECTED_DESIGN = "fat-floor-terminal-native-paraview-comparison-v1"
...
# Direct pvbatch execution is blocked as well as wrapper execution.  Once the
# completed formal outputs are identity-pinned and reviewed, approval may change
# only this boolean.
PARAVIEW_RENDER_EXECUTION_APPROVED_AFTER_RESULT_REVIEW = True
...
def _read_contract(path: Path) -> dict[str, Any]:
    ...
    for key, expected in (
        ...
        if value.get(key) != expected:
            raise ValueError(f"ParaView contract {key} changed")
...
def _render_plate(
    *,
    ...
    layout = pvs.CreateLayout(name=f"fat-floor-terminal-{mode}")
    ...
    for case_id in CASE_ORDER:
        ...
        reader = pvs.XMLPolyDataReader(
            registrationName=f"{case_id} terminal IsFace",
        ...
        if (
            ...
            raise ValueError(f"{case_id} ParaView dimensions changed")
        ...
        if not required <= names:
            raise KeyError(f"{case_id} ParaView point arrays changed")
    ...
    stem = f"26-paraview-terminal-{mode}"
    ...
    if not png_temporary.is_file() or not state_temporary.is_file():
        raise RuntimeError(f"ParaView did not write terminal {mode} outputs")
...
def main() -> None:
    if not PARAVIEW_RENDER_EXECUTION_APPROVED_AFTER_RESULT_REVIEW:
        raise RuntimeError(
            "NO-GO: direct ParaView rendering awaits completed-result identity review"
        )
