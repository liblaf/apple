...
SCHEMA_VERSION = 1
DESIGN = "fixed-rim-local-actuator-inverse-fit-to-smooth-unreachable-target"
...
class Config(cherries.BaseConfig):
    ...
    output_dir: Path = cherries.output("20-bumpy-inverse-belly-losses", mkdir=True)
    steps: int = 500
    ...
    grid_3d: int = 41
    actuators_2d: int = 9
    actuator_axis_3d: int = 5
    actuator_width_2d: float = 0.065
    actuator_width_3d: float = 0.10
    actuator_scale_2d: float = 0.050
    actuator_scale_3d: float = 0.040
    target_depth_2d: float = 0.040
    target_depth_3d: float = 0.035
    bump_filter_spacing_fraction: float = 0.55
    foundation_stiffness: float = 1.0
    membrane_stiffness: float = 0.25
    bending_stiffness: float = 0.015
...
class BellyModel:
    ...
    projection_controls: np.ndarray
...
class CaseResult:
    ...
    bump_highpass_rms: np.ndarray
    bump_laplacian_rms: np.ndarray
    bump_peak_to_valley: np.ndarray
    best_index: int
...
def validate_config(cfg: Config) -> None:
    ...
    if cfg.actuators_2d < 5 or cfg.actuators_2d % 2 == 0:
        raise ValueError("actuators_2d must be an odd integer of at least 5")
    if cfg.actuator_axis_3d < 3 or cfg.actuator_axis_3d % 2 == 0:
        raise ValueError("actuator_axis_3d must be an odd integer of at least 3")
    if min(cfg.actuator_width_2d, cfg.actuator_width_3d) <= 0.0:
        raise ValueError("actuator widths must be positive")
    if min(cfg.actuator_scale_2d, cfg.actuator_scale_3d) <= 0.0:
        raise ValueError("actuator scales must be positive")
    if min(cfg.target_depth_2d, cfg.target_depth_3d) <= 0.0:
        raise ValueError("target depths must be positive")
    if cfg.bump_filter_spacing_fraction <= 0.0:
        raise ValueError("bump filter spacing fraction must be positive")
    if (
        min(
...
def make_smooth_unreachable_target(
    operator: sp.csc_matrix,
    response_interior: np.ndarray,
    smooth_load: np.ndarray,
    target_depth: float,
) -> tuple[np.ndarray, np.ndarray, dict[str, float]]:
    target = -np.asarray(spla.spsolve(operator, smooth_load), dtype=np.float64)
    target *= target_depth / float(np.max(np.abs(target)))
    projection_controls = np.linalg.lstsq(response_interior, target, rcond=None)[0]
    projection = response_interior @ projection_controls
    unreachable = target - projection
    projection_residual = projection - target
    ...
    if projection_rms <= 1.0e-8:
        raise ValueError("the smooth target lies numerically inside the response space")
    certificate = {
        "orthogonality_max_abs": float(np.max(np.abs(orthogonality))),
        "unconstrained_projection_controls": projection_controls.tolist(),
...
def make_2d_model(cfg: Config) -> BellyModel:
    ...
    centers = np.linspace(-0.82, 0.82, cfg.actuators_2d, dtype=np.float64)[:, None]
    width = cfg.actuator_width_2d
    ...
    operator = equilibrium_operator(laplacian, cfg)
    response_interior = normalized_responses(operator, loads, cfg.actuator_scale_2d)
    response = np.zeros((cfg.grid_2d, centers.shape[0]), dtype=np.float64)
    response[interior_indices] = response_interior

    smooth_load = np.cos(0.5 * np.pi * x_interior) ** 2
    target_interior, unreachable_interior, certificate = make_smooth_unreachable_target(
        operator,
        response_interior,
        smooth_load,
        cfg.target_depth_2d,
    )
    projection_controls = np.asarray(
        certificate["unconstrained_projection_controls"], dtype=np.float64
    )
    ...
    metadata: dict[str, Any] = {
        ...
        "target_depth": cfg.target_depth_2d,
        "target_kind": "continuous smooth equilibrium load",
        "bump_filter_sigma_grid": float(
            cfg.bump_filter_spacing_fraction
...
def make_3d_model(cfg: Config) -> BellyModel:
    ...
    center_axis = np.linspace(-0.76, 0.76, cfg.actuator_axis_3d, dtype=np.float64)
    centers = np.asarray(
        [(cx, cz) for cx in center_axis for cz in center_axis], dtype=np.float64
    )
    width = cfg.actuator_width_3d
    loads = np.column_stack(
        [
            np.exp(
                -0.5
    ...
    operator = equilibrium_operator(laplacian, cfg)
    response_interior = normalized_responses(operator, loads, cfg.actuator_scale_3d)
    response = np.zeros((cfg.grid_3d**2, centers.shape[0]), dtype=np.float64)
    response[interior_indices] = response_interior

    smooth_load = np.square(np.cos(0.5 * np.pi * x_interior)) * np.square(
        np.cos(0.5 * np.pi * z_interior)
    )
    target_interior, unreachable_interior, certificate = make_smooth_unreachable_target(
        operator,
        response_interior,
    ...
    metadata: dict[str, Any] = {
        ...
        "actuator_width": width,
        "target_depth": cfg.target_depth_3d,
        "target_kind": "continuous smooth equilibrium load",
        "bump_filter_sigma_grid": float(
            cfg.bump_filter_spacing_fraction
            * (center_axis[1] - center_axis[0])
            / (x[1] - x[0])
        ),
        "rest_height_min": float(rest_height.min()),
        "rest_height_max": float(rest_height.max()),
...
def bumpiness_metrics(model: BellyModel, displacement: np.ndarray) -> dict[str, float]:
    sigma = float(model.metadata["bump_filter_sigma_grid"])
    dx = float(model.x[1] - model.x[0])
    if model.dimension == "2d":
        smooth = ndi.gaussian_filter1d(displacement, sigma=sigma, mode="nearest")
        highpass = displacement - smooth
        selected = highpass[model.interior_indices]
        laplacian = (
            displacement[:-2] - 2.0 * displacement[1:-1] + displacement[2:]
        ) / (dx * dx)
        derivative = np.diff(displacement)
        extrema = int(np.count_nonzero(np.diff(np.sign(derivative))))
    else:
        field = displacement.reshape(model.grid_shape)
        smooth = ndi.gaussian_filter(field, sigma=sigma, mode="nearest")
        highpass = field - smooth
        selected = highpass[1:-1, 1:-1].reshape(-1)
        laplacian = (
            field[:-2, 1:-1]
            + field[2:, 1:-1]
            + field[1:-1, :-2]
        ...
        extrema = -1
    return {
        "highpass_rms": float(np.sqrt(np.mean(np.square(selected)))),
        "laplacian_rms": float(np.sqrt(np.mean(np.square(laplacian)))),
        "highpass_peak_to_valley": float(np.ptp(selected)),
        "extrema_2d": extrema,
    }
...
def optimize_case(
    ...
    bump_highpass_history: list[float] = []
    bump_laplacian_history: list[float] = []
    bump_peak_to_valley_history: list[float] = []
    best_index = 0
    best_objective = math.inf
    target_bumpiness = bumpiness_metrics(model, model.target_displacement)
    ...
    for step in range(cfg.steps + 1):
        ...
        bumpiness = bumpiness_metrics(model, displacement_full)
        ...
        bump_highpass_history.append(bumpiness["highpass_rms"])
        bump_laplacian_history.append(bumpiness["laplacian_rms"])
        bump_peak_to_valley_history.append(bumpiness["highpass_peak_to_valley"])
            ...
            cherries.log_metrics(
                {
                    ...
                    f"{prefix}/bump_highpass_rms": bump_highpass_history[-1],
                    f"{prefix}/bump_laplacian_rms": bump_laplacian_history[-1],
                }
            )
    ...
    bump_highpass_np = np.asarray(bump_highpass_history, dtype=np.float64)
    bump_laplacian_np = np.asarray(bump_laplacian_history, dtype=np.float64)
    bump_peak_to_valley_np = np.asarray(bump_peak_to_valley_history, dtype=np.float64)
    ...
    best_bumpiness = bumpiness_metrics(model, displacements_np[best_index])
    final_bumpiness = bumpiness_metrics(model, displacements_np[-1])
    initial_bumpiness = bumpiness_metrics(model, displacements_np[0])
    reference_bumpiness = bumpiness_metrics(model, reference.displacement)
    ...
    summary: dict[str, Any] = {
        ...
        "bumpiness": {
            "target": target_bumpiness,
            "initial": initial_bumpiness,
            "best": best_bumpiness,
            "final": final_bumpiness,
            "reference": reference_bumpiness,
            "best_highpass_ratio_to_target": float(
                best_bumpiness["highpass_rms"]
                / max(target_bumpiness["highpass_rms"], 1.0e-15)
            ),
        },
        "validation": {
            "finite_history": bool(
            ...
            "bumpiness_emerges": bool(
                best_bumpiness["highpass_rms"]
                > initial_bumpiness["highpass_rms"] + 1.0e-8
            ),
            "best_bumpier_than_smooth_target": bool(
                best_bumpiness["highpass_rms"] > 1.25 * target_bumpiness["highpass_rms"]
            ),
        },
    }
    return CaseResult(
        ...
        worst_ties=ties_np,
        bump_highpass_rms=bump_highpass_np,
        bump_laplacian_rms=bump_laplacian_np,
        bump_peak_to_valley=bump_peak_to_valley_np,
...
def frame_indices(result: CaseResult, cfg: Config) -> list[int]:
    ...
    indices.update(
        step
        for step in (0, 1, 2, 5, 10, 20, result.best_index, cfg.steps)
        if step <= cfg.steps
    )
...
def configure_2d_axes(
    ...
    height_ax.clear()
    residual_ax.clear()
    current = height - model.rest_height
    target = model.target_displacement
    residual = current - target
    height_ax.axhline(0.0, color="#64748B", linestyle=":", linewidth=1.2)
    height_ax.plot(
        model.x,
        target,
        color=TARGET_COLOR,
    ...
    depth = float(np.max(np.abs(target)))
    height_ax.set_xlim(-1.03, 1.03)
    height_ax.set_ylim(-1.55 * depth, 0.15 * depth)
    height_ax.set_ylabel("induced height displacement")
    height_ax.grid(True)
    height_ax.legend(loc="upper right", frameon=True)
    height_ax.set_title(
        f"2D bumpy inverse | {LOSS_LABELS[result.loss_name]} | "
        ...
        f"bump RMS {result.bump_highpass_rms[step_index]:.5f}\n"
        "smooth unreachable target; bumps from discrete local actuators",
        fontsize=10,
    )
    residual_ax.plot(
...
def configure_3d_axes(
    ...
    current = (height - model.rest_height).reshape(model.grid_shape)
    target = model.target_displacement.reshape(model.grid_shape)
    residual = current - target
    ...
    surface_ax.set_zlim(-1.55 * depth, 0.15 * depth)
    surface_ax.set_xlabel("x")
    surface_ax.set_ylabel("z")
    surface_ax.set_zlabel("induced displacement", labelpad=1)
...
def render_case(
    ...
    if model.dimension == "2d":
        ...
        fig.suptitle(
            "3D local-actuator inverse belly: smooth unreachable target; "
            "bumps emerge from discrete actuation"
        )
        ...
        fig.colorbar(
            ...
            ax=residual_ax,
            shrink=0.72,
            label="current - target displacement",
        )
    ...
    for control_id in range(model.n_controls):
        ...
        visible_label = (
            control_label
            if model.dimension == "2d" or abs(float(center[1])) < 1.0e-12
            else "_nolegend_"
        )
        ax.plot(
            result.steps,
    ...
    label_note = "" if model.dimension == "2d" else " (center-z row labeled)"
    ax.set_title(
        f"{model.dimension.upper()} {LOSS_LABELS[result.loss_name]} "
        f"local controls{label_note}"
    )
    ax.grid(True)
    ...
    bumpiness_png = case_dir / "bumpiness-evolution.png"
    target_bumpiness = bumpiness_metrics(model, model.target_displacement)
    reference_bumpiness = bumpiness_metrics(model, result.reference.displacement)
    fig, axes = plt.subplots(1, 2, figsize=(9.0, 4.2), constrained_layout=True)
    axes[0].plot(
        result.steps,
        result.bump_highpass_rms,
        color=LOSS_COLORS[result.loss_name],
        linewidth=1.8,
        label="inverse state",
    )
    axes[0].axhline(
        target_bumpiness["highpass_rms"],
        color=TARGET_COLOR,
        linestyle="--",
        label="smooth target",
    )
    axes[0].axhline(
        reference_bumpiness["highpass_rms"],
        color="#475569",
        linestyle=":",
        label="exact reference",
    )
    axes[0].set_xlabel("optimization step")
    axes[0].set_ylabel("high-pass RMS")
    axes[0].set_title("emergent surface bumpiness")
    axes[0].grid(True)
    axes[0].legend(fontsize=8)
    axes[1].plot(
        result.steps,
        result.bump_laplacian_rms,
        color=LOSS_COLORS[result.loss_name],
        linewidth=1.8,
    )
    axes[1].axhline(
        target_bumpiness["laplacian_rms"],
        color=TARGET_COLOR,
        linestyle="--",
    )
    axes[1].axhline(
        reference_bumpiness["laplacian_rms"],
        color="#475569",
        linestyle=":",
    )
    axes[1].set_xlabel("optimization step")
    axes[1].set_ylabel("Laplacian RMS")
    axes[1].set_title("curvature variation")
    axes[1].grid(True)
    fig.suptitle(
        f"{model.dimension.upper()} {LOSS_LABELS[result.loss_name]} bumpy inverse evolution"
    )
    fig.savefig(bumpiness_png, dpi=180)
    plt.close(fig)

    artifacts = {
        "best_png": str(best_png),
        "control_evolution_png": str(control_png),
        "bumpiness_evolution_png": str(bumpiness_png),
    }
    if cfg.render_video:
...
def write_case_data(
    ...
    with csv_path.open("w", encoding="utf-8", newline="") as stream:
        ...
        for index, step in enumerate(result.steps):
            row: dict[str, Any] = {
                ...
                "bump_highpass_rms": float(result.bump_highpass_rms[index]),
                "bump_laplacian_rms": float(result.bump_laplacian_rms[index]),
                "bump_peak_to_valley": float(result.bump_peak_to_valley[index]),
            }
            row.update(
                {
                    name: float(value)
    ...
    np.savez_compressed(
        ...
        bump_highpass_rms=result.bump_highpass_rms,
        bump_laplacian_rms=result.bump_laplacian_rms,
        bump_peak_to_valley=result.bump_peak_to_valley,
    ...
    summary["artifacts"] = {
        key: str(Path(value).relative_to(case_dir.parents[1]))
...
def plot_metric_comparison(
    ...
    for suffix, step_limit in (("", None), ("-early", 100)):
        ...
        fig.suptitle(
            f"{dimension.upper()} local-actuator bumpy inverse: common metrics{window}\n"
...
def model_artifact(model: BellyModel, output_dir: Path) -> Path:
    ...
    np.savez_compressed(
        ...
        projection_controls=model.projection_controls,
        unreachable_component=model.unreachable_component,
    )
    return path
...
def make_table(cases: list[dict[str, Any]]) -> str:
    ...
    for case in cases:
        ...
        bumpiness = case["bumpiness"]
        lines.append(
            "| {dimension} | {loss} | {step} | {objective:.7g} | {mae:.7g} | "
            "{rmse:.7g} | {linf:.7g} | {reference:.7g} | {gap:.3%} | "
            "{bump:.7g} | {ratio:.2f}x | {switches} |".format(
                ...
                bump=bumpiness["best"]["highpass_rms"],
                ratio=bumpiness["best_highpass_ratio_to_target"],
...
def compare_cases(results: list[CaseResult]) -> dict[str, Any]:
    ...
    endpoints = {
        loss: {
            ...
            "bump_highpass_rms": float(result.bump_highpass_rms[result.best_index]),
            "bump_laplacian_rms": float(result.bump_laplacian_rms[result.best_index]),
        }
...
def main(cfg: Config) -> None:
    ...
    summary: dict[str, Any] = {
        ...
        "complete": True,
        "scope": (
            "reduced fixed-rim linear surface-equilibrium surrogate with local "
            "actuator loads; not volumetric FEM or a calibrated anatomical tissue model"
        ),
        "loss_definitions": {
        ...
        "target_construction": (
            "smooth continuous-load equilibrium target fitted by a finite set of "
            "narrow local actuator responses; its nonzero orthogonal projection "
            "residual certifies that it lies outside their response span"
        ),
