"""Controlled 2-D folding study: aspect, muscle extent, sharing, and Poisson ratio.

This intentionally leaves ``10-run-pork-2d.py`` untouched.  It reuses its
assembled Stable-NH equilibrium core, but makes the experiment-specific mesh,
target, and control grouping explicit.
"""
...
HEIGHT, THICKNESS, CELL_SIZE = 0.05, 0.1, 0.01
...
def cases() -> tuple[Case, ...]:
    return tuple(
        Case(
            f"l{int(length * 100):03d}-{layout}-{mode}-nu{int(nu * 100):02d}",
            length,
            layout,
            mode,
            nu,
        )
        for length in (0.1, 1.0)
...
class Config(cherries.BaseConfig):
    ...
    output_dir: Path = cherries.output("60-pork-folding-2d", mkdir=True)
    cases: str = "all"
    max_steps: int = 600
...
def run_case(case: Case, cfg: Config) -> dict[str, Any]:
    ...
    grid(mesh, a, target_state, 0, HEIGHT, case.length).save(out / "target.vtu")
    ...
    for step in range(cfg.max_steps + 1):
        ...
        try:
            ...
            value, du, _ = loss(mesh, state.u, HEIGHT, "l2", case.length)
        ...
        row = {
            ...
            **metrics(mesh, u, a, HEIGHT, case.length),
        }
        row.update(
            {
        ...
        grid(mesh, a, state, step, HEIGHT, case.length).save(frame)
    ...
    def evaluate_refinement(
        ...
        try:
            value_at_controls, du, _ = loss(
                mesh, state_at_controls.u, HEIGHT, "l2", case.length
            )
    ...
    def record_refinement(
        ...
        row = {
            ...
            **metrics(mesh, accepted_u, accepted_a, HEIGHT, case.length),
        }
        row.update(
            {
                "detF/min": row["min_det_f"],
    ...
    grid(mesh, best_a, best_state, best_step, HEIGHT, case.length).save(
        out / "best.vtu"
    )
    grid(mesh, best_op_a, best_op_state, best_op_step, HEIGHT, case.length).save(
        out / "best-orientation-preserving.vtu"
    )
    final_step = len(rows) - 1
    grid(mesh, a, state, final_step, HEIGHT, case.length).save(out / "final.vtu")
    tail = rows[-50:]
...
def derivative_check(case: Case) -> dict[str, Any]:
    ...
    _value, du, _ = loss(mesh, state.u, HEIGHT, "l2", case.length)
    _, _, _, B, *_ = assembly(mesh, state.u, a, "stable", False, True)
    ...
    for sign in (-1, 1):
        ...
        vals.append(loss(mesh, ss.u, HEIGHT, "l2", case.length)[0])
...
def main(cfg: Config) -> None:
    ...
    checks = (
        [
            derivative_check(Case("check", 0.1, "full", mode, nu))
            for mode in ("per_cell", "shared")
            for nu in (0.35, 0.49)
        ]
    ...
    reports = [run_case(case, cfg) for case in selected]
    write_json(
        cfg.output_dir / "summary.json",
        {
            "design": "2d-folding-full-factorial",
