"""NONSTATIONARY exploratory h=.20 releases from the canonical shared endpoint.
...
class Config(cherries.BaseConfig):
    ...
    fallback_summary: Path = Path("data/30-shared-fallback-h020/summary.json")
    output_dir: Path = cherries.output("40-exploratory-release-h020", mkdir=True)
    cases: str = "h020-shared-release,h020-shared-release_zero_u"
    ...
    *,
    ...
    report = {
        ...
        "inverse": {
            ...
            "best_step": best[1],
            "best_loss": best[0],
            "best_orientation_preserving_step": best_op[1] if best_op[2] is not None else None,
            "best_orientation_preserving_loss": best_op[0] if best_op[2] is not None else None,
            "adam": {
        ...
        "metrics": {
            ...
            "best": rows[best[1]],
            "final": rows[-1],
            "best_orientation_preserving": rows[best_op[1]] if best_op[2] is not None else None,
        },
        ...
        "artifacts": {
            ...
            "target": "target.vtu",
            "best": "best.vtu",
            "best_orientation_preserving": "best-orientation-preserving.vtu" if best_op[2] is not None else None,
            "final": "final.vtu",
...
def main(cfg: Config) -> None:
    ...
    canonical_dir = cfg.canonical_root.resolve() / "h020-shared"
    source = canonical_dir / "final-state.npz"
    summary_path = canonical_dir / "summary.json"
    if not source.is_file() or not summary_path.is_file():
        raise FileNotFoundError("immutable canonical shared endpoint is unavailable")
    shared = json.loads(summary_path.read_text(encoding="utf-8"))
    if shared.get("case", {}).get("name") != "h020-shared":
        raise ValueError("expected canonical h020-shared summary")
    if not cfg.fallback_summary.is_file():
        raise FileNotFoundError("fallback-attempt summary is required for cumulative cost")
    fallback = json.loads(cfg.fallback_summary.read_text(encoding="utf-8"))
    fallback_cost = fallback["fallback"]["cost"]
    reports: list[dict[str, Any]] = []
    for case in selected:
        if not case.protocol.startswith("shared_then_release"):
            raise ValueError(f"exploratory runner only accepts release cases: {case.name}")
        seed = np.load(source)
        ...
        report = run_case(
            ...
            initialization=(
                f"NONSTATIONARY/EXPLORATORY canonical shared activation from {source}; "
                + (
        ...
        first_frame = (
            cfg.output_dir / case.name / "frames" / "step-0000.vtu"
        )
        first_displacement = np.asarray(
            core.pv.read(first_frame).point_data["Displacement"], dtype=float
        )[:, :2].ravel()[mesh.free]
        strict_preflight = shared_u_branch if use_shared_displacement else zero_u_branch
        strict_preflight_u = shared_u_state.u if use_shared_displacement else zero_u_state.u
        normal_first = report["metrics"]["initial"]
        normal_vs_strict = {
            "normal_tolerance": cfg.forward_tolerance,
            "strict_tolerance": cfg.refinement_forward_tolerance,
            "u_inf_delta": float(
                np.linalg.norm(first_displacement - strict_preflight_u, np.inf)
            ),
            "objective_delta": float(
                normal_first["objective"] - strict_preflight["objective"]
            ),
            "determinant_deltas": {
                name: float(normal_first[f"min_{name}"] - strict_preflight[f"min_{name}"])
                for name in ("det_f", "det_ainv", "det_g")
            },
            "asserted_branch_identity": False,
            "interpretation": (
                "This reports the first normal-tolerance release solve against "
                "its strict preflight only; it does not certify branch identity."
            ),
        }
        report["continuation"] = {
            "seed_case": shared["case"]["name"],
            "seed_final_state": str(source),
            "seed_status": "NONSTATIONARY/EXPLORATORY",
            ...
            "handoff": {
                ...
                "normal_tolerance_first_row_vs_strict_preflight": normal_vs_strict,
            },
            "cost": {
                ...
                "cumulative_canonical_shared_fallback_stage2_forward_adjoint": int(
                    shared["inverse"]["cost"]["adam_objective_forward_adjoint_evaluations"]
                    + shared["inverse"]["cost"]["strict_refinement_forward_adjoint_evaluations"]
                    + shared["inverse"]["cost"]["lbfgs_actual_forward_evaluations"]
                    + fallback_cost["aggregate_forward_evaluations"]
                    + report["inverse"]["cost"]["adam_objective_forward_adjoint_evaluations"]
                    + report["inverse"]["cost"]["strict_refinement_forward_adjoint_evaluations"]
                    + report["inverse"]["cost"]["lbfgs_actual_forward_evaluations"]
                    + 3
                ...
                "fallback_attempt": {
                    "summary": str(cfg.fallback_summary.resolve()),
                    "stationary": bool(fallback["fallback"]["stationarity"]),
                    "termination": fallback["fallback"]["termination"],
                    "forward_adjoint_evaluations": fallback_cost["aggregate_forward_evaluations"],
                },
                "comparison_note": (
                    "NONSTATIONARY/EXPLORATORY: comparisons are conditional and "
                    "not convergence-certified"
                ),
            },
        }
    ...
    write_json(
        ...
        {
            "design": "NONSTATIONARY/EXPLORATORY canonical-shared releases",
