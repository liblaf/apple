...
from liblaf.apple.consts import DIRICHLET_MASK, DIRICHLET_VALUE, GLOBAL_POINT_ID
...
def _left_broadcast_to(arr: ArrayLike, shape: Sequence[int]) -> np.ndarray:
    arr = np.asarray(arr)
    arr = np.reshape(arr, arr.shape + (1,) * (len(shape) - arr.ndim))
