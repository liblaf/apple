...
def _arap_muscle_energy_density_func(
    ...
    R, _ = math.polar_rv(F @ A)  # mat33, mat33
    return F.dtype(0.5) * fraction * mu * math.fro_norm_square(F - R @ A)  # float
...
def _arap_muscle_first_piola_kirchhoff_func(
    ...
    mu = materials.mu[cid]  # float
    R, _ = math.polar_rv(F @ A)  # mat33, mat33
    return fraction * mu * (F - R @ A) @ wp.transpose(A)  # mat33
...
def _arap_muscle_hess_diag_func(
    ...
    fraction = materials.fraction[cid]  # float
    mu = materials.mu[cid]  # float
    U, sigma, V = math.svd_rv(F @ A)  # mat33, vec3, mat33
    h4_diag = func.h4_diag(dhdX, U, sigma, A @ V, clamp=clamp)  # mat43
...
def _arap_muscle_hess_prod_func(
    ...
    *,
    ...
    mu = materials.mu[cid]  # float
    U, sigma, V = math.svd_rv(F @ A)  # mat33, vec3, mat33
    h4_prod = func.h4_prod(v, dhdX, U, sigma, A @ V, clamp=clamp)  # mat43
...
def _arap_muscle_hess_quad_func(
    ...
    *,
    ...
    mu = materials.mu[cid]  # float
    U, sigma, V = math.svd_rv(F @ A)  # mat33, vec3, mat33
    h4_quad = func.h4_quad(v, dhdX, U, sigma, A @ V, clamp=clamp)  # float
...
@jarp.frozen_static
class WarpArapMuscleOld(WarpElastic):
