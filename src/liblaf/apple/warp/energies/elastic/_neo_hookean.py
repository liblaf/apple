...
@wp.func
@no_type_check
def _neo_hookean_energy_density_func(
...
@wp.func
@no_type_check
def _neo_hookean_first_piola_kirchhoff_func(
    F: mat33, materials: Materials, cid: int_
) -> mat33:
    fraction = materials.fraction[cid]
    mu = materials.mu[cid]
    ...
    log_J = wp.log(J)
    g3 = func.g3(F)
    return fraction * (mu * F + ((-mu + lambda_ * log_J) / J) * g3)


@wp.func
@no_type_check
def _neo_hookean_hess_diag_func(
    F: mat33,
    dhdX: mat43,
    materials: Materials,
    cid: int_,
...
@wp.func
@no_type_check
def _neo_hookean_hess_prod_func(
    F: mat33,
    v: mat43,
    dhdX: mat43,
    materials: Materials,
...
@wp.func
@no_type_check
def _neo_hookean_hess_quad_func(
    F: mat33,
    v: mat43,
    dhdX: mat43,
    materials: Materials,
...
@jarp.frozen_static
class WarpNeoHookean(WarpElastic):
    energy_density_func: ClassVar[wp.Function] = cast(
        "wp.Function", _neo_hookean_energy_density_func
    )
    first_piola_kirchhoff_func: ClassVar[wp.Function] = cast(
        "wp.Function", _neo_hookean_first_piola_kirchhoff_func
    )
    hess_diag_func: ClassVar[wp.Function] = cast(
        "wp.Function", _neo_hookean_hess_diag_func
    )
    hess_prod_func: ClassVar[wp.Function] = cast(
        "wp.Function", _neo_hookean_hess_prod_func
    )
    hess_quad_func: ClassVar[wp.Function] = cast(
        "wp.Function", _neo_hookean_hess_quad_func
    )
