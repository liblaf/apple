...
@wp.func
@no_type_check
def _stable_neo_hookean_muscle_energy_density_func(
...
@wp.func
@no_type_check
def _stable_neo_hookean_muscle_first_piola_kirchhoff_func(
    F: mat33, materials: Materials, cid: int_
) -> mat33:
    A = func.make_activation_mat33(materials.activation[cid])
    fraction = materials.fraction[cid]
    ...
    return fraction * (mu * G + c * func.g3(G)) @ wp.transpose(A)


@wp.func
@no_type_check
def _stable_neo_hookean_muscle_hess_diag_func(
    F: mat33,
    dhdX: mat43,
    materials: Materials,
    cid: int_,
...
@wp.func
@no_type_check
def _stable_neo_hookean_muscle_hess_prod_func(
    F: mat33,
    v: mat43,
    dhdX: mat43,
    materials: Materials,
...
@wp.func
@no_type_check
def _stable_neo_hookean_muscle_hess_quad_func(
    F: mat33,
    v: mat43,
    dhdX: mat43,
    materials: Materials,
...
@jarp.frozen_static
class WarpStableNeoHookeanMuscle(WarpElastic):
    energy_density_func: ClassVar[wp.Function] = cast(
        "wp.Function", _stable_neo_hookean_muscle_energy_density_func
    )
    first_piola_kirchhoff_func: ClassVar[wp.Function] = cast(
        "wp.Function", _stable_neo_hookean_muscle_first_piola_kirchhoff_func
    )
    hess_diag_func: ClassVar[wp.Function] = cast(
        "wp.Function", _stable_neo_hookean_muscle_hess_diag_func
    )
    hess_prod_func: ClassVar[wp.Function] = cast(
        "wp.Function", _stable_neo_hookean_muscle_hess_prod_func
    )
    hess_quad_func: ClassVar[wp.Function] = cast(
        "wp.Function", _stable_neo_hookean_muscle_hess_quad_func
    )
    ...
    def make_materials(cls, region: Region, requires_grad: Sequence[str]) -> Any:
        @wp.struct
        class WarpStableNeoHookeanMuscleMaterials:
        ...
        materials = WarpStableNeoHookeanMuscleMaterials()
