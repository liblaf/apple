...
@wp.func
@no_type_check
def h1_quad(p: mat43, dhdX: mat43, g1: mat33) -> floating:
    """$p^T h_1 p$."""
...
@wp.func
@no_type_check
def h2_quad(p: mat43, dhdX: mat43, g2: mat33) -> floating:
    """$p^T h_2 p$."""
...
@wp.func
@no_type_check
def h3_quad(p: mat43, dhdX: mat43, g3: mat33) -> floating:
    """$p^T h_3 p$."""
...
@wp.func
def h4_quad(
    p: mat43, dhdX: mat43, U: mat33, sigma: vec3, V: mat33, *, clamp_lambda: bool = True
) -> floating:
    """$p^T h_4 p$."""
    dFdx_p = deformation_gradient_jvp(dhdX, p)  # mat33
    lambdas = _misc.lambdas(sigma, clamp=clamp_lambda)  # vec3
...
@wp.func
def h5_quad(p: mat43, dhdX: mat43) -> floating:
    """$p^T h_5 p$."""
    dFdx_p = deformation_gradient_jvp(dhdX, p)  # mat33
...
@wp.func
@no_type_check
def h6_quad(p: mat43, dhdX: mat43, F: mat33) -> floating:
    """$p^T h_6 p$."""
    dFdx_p = deformation_gradient_jvp(dhdX, p)  # mat33
    f0 = F[:, 0]
    f1 = F[:, 1]
