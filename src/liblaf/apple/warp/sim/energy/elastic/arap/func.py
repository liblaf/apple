...
def energy_density(F: mat33, params: Params) -> None:
    ...
    Psi = 0.5 * params.mu * math.frobenius_norm_square(F - R)  # float
    return Psi
...
@wp.func
@no_type_check
def energy_density_hess_diag(F: mat33, params: Params, dhdX: mat43) -> mat33:
    ...
    h_diag = -float_(2.0) * h4_diag + h5_diag  # mat43
    return 0.5 * params.mu * h_diag  # mat43


@wp.func
@no_type_check
def energy_density_hess_quad(F: mat33, p: mat43, params: Params, dhdX: mat43) -> float:
    U, s, V = math.svd_rv(F)  # mat33, vec3, mat33
    lambdas = utils.lambdas(s)  # vec3
    Q0, Q1, Q2 = utils.Qs(U, V)  # mat33, mat33, mat33
    h4_quad = utils.h4_quad(p=p, dhdX=dhdX, lambdas=lambdas, Q0=Q0, Q1=Q1, Q2=Q2)
    h5_quad = utils.h5_quad(p=p, dhdX=dhdX)
    h_quad = -float_(2.0) * h4_quad + h5_quad
    return 0.5 * params.mu * h_quad
