...
@wp.func
@no_type_check
def h1_quad(p: mat43, dhdX: mat43, g1: mat33) -> float:
    return math.square(wp.ddot(deformation_gradient_jvp(dhdX, p), g1))


@wp.func
@no_type_check
def h2_quad(p: mat43, dhdX: mat43, g2: mat33) -> float:
    return math.square(wp.ddot(deformation_gradient_jvp(dhdX, p), g2))


@wp.func
@no_type_check
def h3_quad(p: mat43, dhdX: mat43, g3: mat33) -> float:
...
@no_type_check
def h4_quad(
    p: mat43, dhdX: mat43, *, lambdas: wp.vec3, Q0: mat33, Q1: mat33, Q2: mat33
) -> float:
...
@wp.func
@no_type_check
def h5_quad(p: mat43, dhdX: mat43) -> float:
    dFdx_p = deformation_gradient_jvp(dhdX, p)  # mat33
...
@wp.func
@no_type_check
def h6_quad(p: mat43, F: mat33, dhdX: mat43) -> float:
    dFdx_p = deformation_gradient_jvp(dhdX, p)  # mat33
    f0 = F[:, 0]
    f1 = F[:, 1]
    f2 = F[:, 2]
